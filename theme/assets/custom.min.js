...
// }
var productFormApp=Vue.createApp({data:function data(){return{isAdding:!1,engraveStyle:"",engraveText:""}},methods:{addToCart:function addToCart(a){var b=this,c=a.target,d=c.dataset,e=d.product,f=d.qty,g=new URLSearchParams(window.location.search),h=g.get("variant"),i=document.getElementById("engrave_style")?document.getElementById("engrave_style").value:"",j=document.getElementById("engrave_text")?document.getElementById("engrave_text").value:"";this.isAdding=!0;fetch("".concat(window.Shopify.routes.root,"cart/add.js"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:[{"product-id":e,id:h||e,quantity:f,properties:{"Engrave style":i,"Engrave text":j}}]})}).then(function(a){a.json(),b.isAdding=!1,updateCartDrawer()}).then(function(){mountedHeaderApp.$data.isCartOpen=!0})["catch"](function(a){console.error("Error:",a)})}}});productFormApp.config.compilerOptions.delimiters=["[[","]]"],productFormApp.mount(".product-form__add");
